...
async function main() {
  try {
    ...
    console.log("🧹 Truncating missions table and restarting identity...");
    ...
    await sql.query("TRUNCATE TABLE missions RESTART IDENTITY CASCADE;");
    ...
    await seed(db, { missions }, { seed: SEED }).refine((funcs) => ({
      missions: {
        count: SEED_COUNT,
        columns: {
    ...
    console.log(`✅ Inserted ${SEED_COUNT} mission(s) into the database\n`);

    // Ensure the missions sequence is synced to the current MAX(id). This is a
    // safety measure in case the DB was imported or mutated in a way that left
    // the sequence behind the table's max value.
    try {
      await sql.query(
        `SELECT setval(pg_get_serial_sequence('missions','id'), COALESCE((SELECT MAX(id) FROM missions), 1), true);`,
      );
      console.log("✅ Sequence synced after seeding");
    } catch (err) {
      console.warn("⚠️ Failed to sync missions sequence after seeding:", err);
    }
  } catch (err) {
